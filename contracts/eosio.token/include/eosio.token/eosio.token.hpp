...
namespace eosio {
    ...
    public:
         ...
         * Recharge action.
         *
         * @details Generate `quantity` tokens for the `to` account and `fee` tokens to reward producers.
         *
         * @param tx_id_hex - the cross chain transaction(tx1) id to refer to in hexadecimal format,
         * @param to - the account to be transferred to,
         * @param quantity - the quantity of tokens to be transferred,
         * @param fee - the transfer fee to reward producers.
         */
        [[eosio::action]]
        void recharge(const string &tx_id_hex,
                      const name &to,
                      const asset &quantity,
                      const asset &fee);
         ...
         * Withdraw action.
         *
         * @details Allows `from` account to withdraw `quantity` tokens to `to` account on main chain,
         * the `fee` is pay for the tx4 transaction fee on main chain.
         *
         * @param from - the account to withdraw from,
         * @param to - the main chain account to be transferred to,
         * @param quantity - the quantity of tokens to be transferred,
         * @param fee - the tx4 transaction fee on main chain.
         */
        [[eosio::action]]
        ...
        void from_hex(const string &hex_str, char *out_data) {
            check(hex_str.size() == 64, "transaction id hex not equal to 64 characters");
        ...
        using recharge_action = eosio::action_wrapper<"recharge"_n, &token::recharge>;
    ...
    private:
        ...
        struct [[eosio::table]] _recharge {
            uint64_t tx_id_part;

            uint64_t primary_key() const { return tx_id_part; }
        };

        typedef eosio::multi_index<"accounts"_n, account> accounts;
        typedef eosio::multi_index<"stat"_n, currency_stats> stats;
        typedef eosio::multi_index<"idparts1"_n, _recharge> id_parts_1;
        typedef eosio::multi_index<"idparts2"_n, _recharge> id_parts_2;
        typedef eosio::multi_index<"idparts3"_n, _recharge> id_parts_3;
        typedef eosio::multi_index<"idparts4"_n, _recharge> id_parts_4;

        void sub_balance(const name &owner, const asset &value);

        void add_balance(const name &owner, const asset &value, const name &ram_payer);

        bool add_tx_id(const string &tx_id_hex);
